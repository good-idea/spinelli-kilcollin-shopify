...
function transformTable(tableElement, publisher, config) {
	...
	const frame = $('<div class="carousel__frame">').appendTo(mainContainer)
	const slidesContainer = $('<div class="carousel__slides">').appendTo(frame)
...
function stripEmptyBeginningTags(article) {
	...
	for (let i = 0; i < children.length; i += 1) {
		...
		if (
			...
			(child.innerHTML === '' || child.innerText.trim() === '')
		) {
			child.remove()
		} else {
			return true
...
function buildBlogCarousels(publisher) {
	...
	$('section.article table').each((i, table) => {
		...
		const config = isFirstChild
			? { container: placeholder, class: 'top-carousel' }
			: {}
